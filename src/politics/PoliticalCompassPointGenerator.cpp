...
std::pair<size_t, double> PoliticalCompassDistribution::sample(std::map<PoliticalAxis, size_t> otherResults) {
...
PoliticalCompassPoint PoliticalCompassPointGenerator::sample() {
    ...
    for (auto axisPair : politicalCompassDists) {
        ...
        ideology[axisPair.first] = result.second;
        previousResults[axisPair.first] = result.first;
        std::cout << axisPair.first << " " <<  result.first << std::endl;
    }
    PoliticalCompassPoint p(ideology, PoliticalCompassPoint::getDefaultStrengths());
    return p;
}
...
PoliticalCompassPointGenerator PoliticalCompassPointGenerator::testSetup() {
    ...
    auto capitalistGaussian = PoliticalCompassDistribution::generateGaussians(0.1, 0.25, 0.35);
    auto conservativeGaussian = PoliticalCompassDistribution::generateGaussians(0.1, 0.25, 0.4);
    auto nationalistGaussian = PoliticalCompassDistribution::generateGaussians(0.05, 0.3, 0.4);
